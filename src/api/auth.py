...
def _schedule_confirmation_email(
    ...
    """
    Приватний хелпер: бере новоствореного користувача (ORM‑модель, Pydantic‑схему чи dict),
    дістає звідти email та username і додає фонове завдання send_email.
    """
...
async def register_user(
    ...
    _schedule_confirmation_email(new_user, background_tasks, request.base_url)
...
async def login_user(
    ...
    if not user or not Hash().verify_password(form_data.password, password):
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def request_email(
    ...
    if user and confirmed:
        return {"message": "Ваша електронна пошта вже підтверджена"}
    if user:
        _schedule_confirmation_email(user, background_tasks, request.base_url)
...
async def confirm_reset_password(token: str, db: AsyncSession = Depends(get_db)):
    ...
    await user_service.reset_password(user_id, hashed_password)
